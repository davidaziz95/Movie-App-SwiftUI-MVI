//
//  SeeMoreModifier.swift
//  Medsulto
//
//  Created by David A. Mikhael on 17/06/2022.
//

import SwiftUI

struct SeeMoreModifier: View {
    ...
    var body: some View {
        ...
        ZStack(alignment: .bottomLeading) {
            ...
            .font(Font(font))
            .foregroundColor(.black)///set default font
            if truncated {
                Button(action: {
                    expanded.toggle()
                }, label: {
...
struct SeeMoreModifier_Previews: PreviewProvider {
  static var previews: some View {
    VStack(alignment: .leading, spacing: 10) {
        ...
        SeeMoreModifier("Small text", lineLimit: 3)
        SeeMoreModifier("Render the limited text and measure its size, R", lineLimit: 1)
        SeeMoreModifier("Create a ZStack with unbounded height to allow the inner Text as much, Render the limited text and measure its size, Hide the background Indicates whether the text has been truncated in its display.", lineLimit: 3)
